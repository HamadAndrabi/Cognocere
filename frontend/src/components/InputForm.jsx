        ...
        >
          ...
          <div className="flex items-center justify-end mt-2">
            ...
            <div className="relative mr-2" ref={modelDropdownRef}>
              <button
                type="button"
                className="flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 hover:border-gray-400 dark:hover:border-gray-500 shadow-sm transition-all duration-150 text-gray-800 dark:text-gray-100 font-medium"
              ...
              >
                ...
                <FaChevronDown className={`h-3 w-3 text-gray-500 transition-transform duration-200 ${isModelDropdownOpen ? 'transform rotate-180' : ''}`} />
              </button>

              {/* Dropdown Menu */}
              {isModelDropdownOpen && (
                <div className="absolute z-50 mt-1 w-72 max-h-[320px] overflow-y-auto rounded-md bg-white dark:bg-gray-800 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none custom-scrollbar p-1">
                  <div className="space-y-1">
                    {Object.entries(groupedModels).map(([provider, providerModels], index) => (
                      <div key={provider} className={`${index > 0 ? 'mt-2' : ''}`}>
                        {/* Provider Header */}
                        <div className="px-3 py-1.5 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 flex items-center gap-2">
                        ...
                        {providerModels.map(model => (
                          <button
                            ...
                            className={`w-full px-3 py-2 text-left text-sm rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center justify-between group ${ 
